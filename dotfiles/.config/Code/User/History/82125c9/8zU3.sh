...
# --- VARIABLES ---
REPO_URL="git@github.com:TU_USUARIO/dotfiles.git" # Cambia esto a la URL de tu repositorio
...
install_packages() {
    ...
    if [[ "$response" =~ ^([sS][iI]|[sS])$ ]]; then
        ...
        sudo apt install -y kitty zsh fzf starship bat eza

        # Lenguajes de programación
        sudo apt install -y python3 python3-pip golang rustc cargo

        # Herramientas de desarrollo
        sudo apt install -y git github-cli code
