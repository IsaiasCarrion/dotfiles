...
# Forzar instalación de zsh sin interacción
sudo apt update && sudo apt install -y zsh

# Cambiar shell en Docker no tiene efecto, usar zsh directamente al entrar
echo "Para iniciar Zsh: docker exec -it test-dotfiles zsh"

# Verificar si el archivo fue copiado correctamente
if [[ -f "$HOME/.zshrc" ]]; then
    echo "~/.zshrc copiado correctamente."
else
    echo "Error: ~/.zshrc no se copió."
fi
